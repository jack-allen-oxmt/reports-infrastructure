...
from fixtures import fake_urlopen_success, fake_urlopen_failure

def test_send_events_success(fake_urlopen_success):
...
def test_send_events_failure(fake_urlopen_failure):
    docs = [{'timestamp': 1600000000000, 'path': '/a', 'session_id': 's1', 'user': 'u1'}]
    count = send_events_to_opensearch(docs, 'search.example.com', 'ap-southeast-2')
    assert count == 0
...
def test_lambda_handler_processing(monkeypatch, fake_urlopen_success):
    # Patch environment
